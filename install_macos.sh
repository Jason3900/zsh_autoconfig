...
setup_ohmyzsh() {
  ...
  mkdir $ZSH && tar zxvf ohmyzsh.tar.gz -C $ZSH 2>&1 > /dev/null || {
...
config_zsh() {
  echo "${BLUE}config zsh...${RESET}"
  sed -i '' 's/# DISABLE_AUTO_UPDATE="true"/DISABLE_AUTO_UPDATE="true"/g' ${HOME}/.zshrc
  sed -i '' 's/# DISABLE_UPDATE_PROMPT="true"/DISABLE_UPDATE_PROMPT="true"/g' ${HOME}/.zshrc
  echo "setopt no_nomatch" >> ${HOME}/.zshrc
  sed -i '' 's/ZSH_THEME="robbyrussell"/ZSH_THEME="steeef"/g' ${HOME}/.zshrc
}
 
install_plugins() {
  echo "${BLUE}install zsh-plugins: autosuggestions syntax-highlighting...${RESET}"
  tar zxvf plugins.tar.gz 2>&1 > /dev/null
  cp -r ./zsh-autosuggestions ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-autosuggestions
